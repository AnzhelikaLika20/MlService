...
tab = st.sidebar.radio("Выберите режим", ["Datasets", "Training", "Inference", "Status"])
...
if tab == "Datasets":
    ...
    st.info("Список датасетов, загруженных через DVC. Загрузите CSV или JSON (строго CSV с header или JSON-список объектов).")
...
#
    ...
    st.info("Выберите модель и датасет (DVC, upload или raw JSON). Настройте фичи/таргет и гиперпараметры.")

    supported = _safe_get("/models/supported-models")
    models_list = supported.get("models") if supported else []
    if not models_list:
        st.warning("Не удалось получить список моделей или список пуст. Проверьте /models/supported-models")
        models_list = []

    model_choice = st.selectbox("Model class", options=models_list)

    st.markdown("### Data input")
    train_mode = st.radio("Source of training data", [
        "Select DVC dataset",
    ...
    if train_mode == "Select DVC dataset":
        ...
        if isinstance(dvc_datasets, dict) and dvc_datasets.get("error"):
            ...
            dvc_datasets = []
        ds_name = st.selectbox("Выбери датасет (DVC)", options=dvc_datasets if dvc_datasets else [])
        if ds_name:
        ...
        file = st.file_uploader("Upload dataset for training", type=["csv", "json"], key="trainfile")
        if file:
            st.markdown("Preview")
            try:
        ...
        if not model_choice:
            ...
            if train_mode in ["Select DVC dataset", "Upload file (CSV/JSON)"]:
                if df is None:
                    ...
                    try:
                        if not feature_cols:
                            st.error("Выберите хотя бы одну колонку как признак (feature)")
                            st.stop()
                        if not target_col:
                        ...
                        with st.spinner("Training..."):
                            res = _safe_post(f"/models/{model_choice}/train", json_payload=payload)
                        if res and res.get("status") == "trained":
                            st.success(f"Trained. Model id: {res.get('model_id')}, name: {res.get('name')}")
                        else:
...
#
    ...
    st.title("Inference")
    st.info("Выберите модель (из ClearML), используйте DVC-датасет или введите X вручную.")
    ...
    if use_dvc_ds:
        dvc_datasets = _safe_get("/datasets")
        ds_name = st.selectbox("Выбери DVC датасет", options=dvc_datasets if dvc_datasets else [], key="inferdataset")
        if ds_name:
            df = _load_dvc_dataset(ds_name)
            if df is not None:
                st.dataframe(df.head())
    ...
    if st.button("Predict"):
        if not model_choice:
            ...
            if X is None:
                ...
                backend_model_name = model_map.get(model_choice, {}).get("name") or model_choice
                with st.spinner("Requesting predictions..."):
                    res = _safe_post(f"/models/{backend_model_name}/predict", json_payload={"X": X})
    ...
    st.write("При проблемах проверьте выполнение шагов из README, переменные окружения и доступность ClearML")
